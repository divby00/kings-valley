...
func _process(_delta):
	...
	if Input.is_action_just_pressed("ui_home") && Vick.visible:
		if (
			! get_tree().paused
			and not Vick.state in [Vick.st_dying, Vick.st_init, Vick.st_goingout, Vick.st_leveldone]
		):
			emit_signal("sig_show_gameoptions")


func set_level(l, pfrom):
	level = l
	from_level = pfrom
	read_level(level)


func draw_tile(layer, x, y, id):
	layer.set_cell(x, y, 0, false, false, false, Vector2(id % 20, int(id / 20)))


func draw_front_tile(x, y, id):
	layer_front.set_cell(x, y, 0, false, false, false, Vector2(id % 20, int(id / 20)))


func clear_tile(layer, x, y):
	layer.set_cell(x, y, -1)


func get_buffer_value(buffer, x, y) -> int:
	if y > height - 1:
		y = height - 1
	return buffer[y * width + x]


func set_buffer_value(buffer, x, y, value) -> void:
	buffer[y * width + x] = value


func read_level(plevel):
	var file = File.new()
	...
	for y in range(0, height):
		for x in range(0, width):
			var value = get_buffer_value(frontBuffer, x, y)
			if value in [23, 24, 27, 28, 31, 32, 35, 36, 43, 44, 47, 48, 51, 52, 55, 56]:
				set_buffer_value(frontBuffer, x, y, 0)
				set_buffer_value(backBuffer, x, y, value)
...
func draw_buffer_on_layer(buffer, layer):
	for y in range(0, height):
		for x in range(0, width):
			var value = get_buffer_value(buffer, x, y)
			if value > 0:
				draw_tile(layer, x, y, value - 1)
			else:
				clear_tile(layer, x, y)
...
func draw_pyramid():
	...
	layer_front.clear()
	layer_pick.clear()
	remove_children(jewels_node)
	remove_children(daggers_node)
	remove_children(pickers_node)
	remove_children(doors_node)
	remove_children(giradoors_node)
	remove_children(stairs_node)
	remove_children(walls_node)
	remove_children(mummies_node)
	jewels = 0
	mummies_node.visible = false
	draw_buffer_on_layer(backBuffer, layer_back)
	draw_buffer_on_layer(frontBuffer, layer_front)
	for item in items:
		match item.type:
			Globals.It_jewel:
			...
			Globals.It_dagger:
				put_dagger_on_floor(item.x, item.y, item.p0 - 1)
...
func put_dagger_on_floor(px, py, frame):
...
func dagger_on_floor(dagger):
	...
	while not is_cell_free_for_dagger(pos):
		if dagger.is_flip():
			dagger.position.x -= 5
		else:
			dagger.position.x += 5
...
# warning-ignore:narrowing_conversion
	while pos.y < height - 1 and not is_brick(pos.x, pos.y + 1):
		pos.y += 1
	put_dagger_on_floor(pos.x, pos.y, 175)
	dagger.queue_free()


#Determina si una celdilla esta libre para poder poner un item
func is_cell_free_for_dagger(position: Vector2) -> bool:
	var ipos: Vector2
...
func is_giratory_or_wall(position: Vector2) -> bool:
	var ipos: Vector2
...
func is_pickable_brick(px: int, py: int) -> bool:
	var brick = get_buffer_value(frontBuffer, px, py)
	return ! (brick in [0, 21, 22, 25, 26, 29, 30, 33, 34, 41, 42, 45, 46, 49, 50, 53, 54])


func is_brick(px: int, py: int) -> bool:
	return get_buffer_value(frontBuffer, px, py) > 0


func is_locked(sprite) -> bool:
	...
	return (
		(is_brick(int(pos.x - 1), int(pos.y - 1)) and is_brick(int(pos.x + 1), int(pos.y - 1)))
		or (is_brick(int(pos.x - 1), int(pos.y)) and is_brick(int(pos.x + 1), int(pos.y)))
	)


func is_floor_down(sprite) -> bool:
	var pos: Vector2 = sprite.position
	pos.x = int(pos.x / 10)
	pos.y = int(pos.y / 10)
	return is_brick(int(pos.x), int(pos.y + 1))


func can_jump_left(sprite) -> bool:
	var pos: Vector2 = sprite.position
	pos.x = int(pos.x / 10)
	pos.y = int(pos.y / 10)
	if pos.x <= 1:
		return false
	return not (
		is_brick(int(pos.x - 1), int(pos.y - 1))
		or is_brick(int(pos.x - 1), int(pos.y - 2))
	)


func is_floor_left(sprite) -> bool:
	var pos: Vector2 = sprite.position
...
func can_jump_right(sprite) -> bool:
	var pos: Vector2 = sprite.position
	pos.x = int(pos.x / 10)
	pos.y = int(pos.y / 10)
	if pos.x >= width - 1:
...
func is_floor_right(sprite) -> bool:
	var pos: Vector2 = sprite.position
	pos.x = int((pos.x + 1) / 10)
	pos.y = int(pos.y / 10)
	return is_brick(int(pos.x), int(pos.y + 1))
...
func teleport(sprite):
	...
	while not done:
		...
		done = not is_brick(cx, cy) and dist > 8
		done = done and not is_brick(cx + 1, cy)
		done = done and not is_brick(cx, cy - 1)
		done = done and not is_brick(cx + 1, cy - 1)

	# Bajamos hasta tocar suelo
	while not is_brick(cx, cy + 1):
		cy += 1
	sprite.position.x = cx * 10 + 5
	sprite.position.y = cy * 10 + 9


func where_can_vick_pick() -> Array:
	...
	if (
		is_pickable_brick(int(pos.x - 1), int(pos.y - 1))
		and is_pickable_brick(int(pos.x + 1), int(pos.y - 1))
	):
		...
		if Vick.is_flip():
			if pos.x <= 1:
				...
				if is_pickable_brick(int(pos.x - 1), int(pos.y)):
		...
		if is_giratory_or_wall(Vector2(pos.x, pos.y + 1)):
			return result

		#Si delante hay un item, no se puede picar
		if not is_cell_free_for_dagger(Vector2(pos.x, pos.y)):
			return result

		#Si delante hay un muro, no se puede picar
		if is_pickable_brick(int(pos.x), int(pos.y)):
			return result

		#Si debajo no hay muro, no se puede picar
		if not is_pickable_brick(int(pos.x), int(pos.y + 1)):
			return result

		#Podemos picar
		result.append(Vector2(pos.x, pos.y + 1))
		if pos.y < height - 3 and is_pickable_brick(int(pos.x), int(pos.y + 2)):
			result.append(Vector2(pos.x, pos.y + 2))

	return result


func pick_cell(cell: Vector2, frame: int):
	if frame < 4:
		draw_tile(layer_pick, cell.x, cell.y, 176 + frame)
	else:
		clear_tile(layer_pick, cell.x, cell.y)
		clear_tile(layer_front, cell.x, cell.y)
		set_buffer_value(frontBuffer, cell.x, cell.y, 0)


func set_vick_initial_position():
	for d in doors_node.get_children():
		if (
			(from_level <= level and d.door_type in [TDoor.TYPE.IN, TDoor.TYPE.BOTH])
			or (from_level > level and d.door_type in [TDoor.TYPE.OUT, TDoor.TYPE.BOTH])
		):
			Vick.position = d.position + d.get_vick_position()


func show_doors(enterlevel: bool):
	var wait_door = null
	for d in doors_node.get_children():
