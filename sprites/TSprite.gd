		...
		st_onstairs:
			...
			if stair_init in [TStairDetect.STAIR_TYPE.DOWN_LEFT,TStairDetect.STAIR_TYPE.UP_RIGHT]:
				if input_vector.x < 0:
					...
					motion.y = -speed_walk_stairs
			elif stair_init in [TStairDetect.STAIR_TYPE.DOWN_RIGHT,TStairDetect.STAIR_TYPE.UP_LEFT]:
				if input_vector.x < 0:
					motion.y = -speed_walk_stairs
				elif input_vector.x > 0:
					motion.y = speed_walk_stairs
...
func doMove(_delta):
	...
	if state in [st_dying,st_walk,st_jump_left,st_jump_right,st_jump_top,st_onstairs,st_goingout,st_giratory]:
	...
	match state:
		st_walk:
			if stair_cell.stair_down!=null and input_vector.y > 0:
				if not ((stair_cell.stair_down.stair_type==TStairDetect.STAIR_TYPE.DOWN_LEFT and input_vector.x < 0) \
					or (stair_cell.stair_down.stair_type==TStairDetect.STAIR_TYPE.DOWN_RIGHT and input_vector.x > 0)):
					return
				...
				doBrickCollision(false)
				return
			
			if stair_cell.stair_up!=null and input_vector.y < 0:
				if not ((stair_cell.stair_up.stair_type==TStairDetect.STAIR_TYPE.UP_LEFT and input_vector.x < 0) \
					or (stair_cell.stair_up.stair_type==TStairDetect.STAIR_TYPE.UP_RIGHT and input_vector.x > 0)):
					return
				state = st_onstairs
		...
		st_onstairs:
			if stair_init in [TStairDetect.STAIR_TYPE.DOWN_LEFT,TStairDetect.STAIR_TYPE.UP_RIGHT]:
				if input_vector.x > 0:
					if stair_cell.stair_down!=null:
						if stair_cell.stair_up!=null and stair_cell.stair_up.stair_type==TStairDetect.STAIR_TYPE.UP_RIGHT and input_vector.y < 0:
							return
						else:
							state = st_walk
							motion.y = 0
					...
					if stair_cell.stair_up!=null:
						if stair_cell.stair_down!=null and stair_cell.stair_down.stair_type==TStairDetect.STAIR_TYPE.DOWN_LEFT and input_vector.y > 0:
							return
						else:
							state = st_walk
							motion.y = 0
			...
			if stair_init in [TStairDetect.STAIR_TYPE.DOWN_RIGHT,TStairDetect.STAIR_TYPE.UP_LEFT]:
				if input_vector.x < 0:
					if stair_cell.stair_down!=null:
						if stair_cell.stair_up!=null and stair_cell.stair_up.stair_type==TStairDetect.STAIR_TYPE.UP_LEFT and input_vector.y < 0:
							return
						else:
							state = st_walk
							motion.y = 0
					...
					if stair_cell.stair_up!=null:
						if stair_cell.stair_down!=null and stair_cell.stair_down.stair_type==TStairDetect.STAIR_TYPE.DOWN_RIGHT and input_vector.y > 0:
							return
						else:
							state = st_walk
							motion.y = 0
